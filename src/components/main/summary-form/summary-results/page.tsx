...
export default function SummaryResults() {
  ...
  const copyToClipboard = (e: any) => {
    ...
    const summaryContent = summary.current.textContent;
  ...
  return (
    <div className="w-full lg:w-[700px] min-h-[300px] relative flex justify-center items-center flex-col gap-4 px-8 pt-20 pb-12 outline-none border-white border border-opacity-30 rounded-lg bg-primary-200 text-white mt-8">
      <Button
        ...
        disabled={!summaryCtx?.state.isStreaming}
      ...
      {summaryCtx?.state.isLoading ? (
        ...
        <>
          ...
          <p className="prose prose-lg prose-neutral text-white" ref={summary}>
            {summaryCtx?.state.summarizedText ? (
              <Markdown>{summaryCtx?.state.summarizedText}</Markdown>
            ) : (
