...
import { FormEvent, useMemo, useReducer, createContext } from "react";
...
export interface ISummaryAction {
  type: "RESET" | "SET_IS_STREAMING" | "SET_SUMMARIZED_TEXT" | "SET_URL_INPUT" | "SET_PARAGRAPH_INPUT" | "SET_IS_CHECKED" ;
  payload?: any;
}
...
const summaryReducer = (
  ...
  switch (action.type) {
    ...
    case "SET_SUMMARIZED_TEXT":
      return {
        ...state,
        summarizedText: state.summarizedText + action.payload,
      };
    case "SET_URL_INPUT":
      return { ...state, urlInput: action.payload };
    case "SET_PARAGRAPH_INPUT":
      return { ...state, paragraphInput: action.payload };
    case "SET_IS_CHECKED":
...
export const SummaryContext = createContext<ContextType | null>(null);
...
export default function SummaryForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const url = state.checked ? "/api/url" : "/api/text";
    const payload = state.checked ? state.urlInput : state.paragraphInput;

    await request.POST(url, payload, state, dispatch);
  ...
  return (
    <SummaryContext.Provider value={value}>
      ...
      >
        {state.checked ? <SummaryFormUrl /> : <SummaryFormParagraph />}
      </form>
      <SummaryResults />
